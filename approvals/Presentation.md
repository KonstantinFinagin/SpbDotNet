...
- Использование тесно связано с **Dependency Injection**
...
- Логика: рефакторинги, легаси, сначала придумали логику)
- Еще нет логики : TDD
- Логика упала на проде, и мы хотим её воспроизвести и зафиксировать в тесте (реальная жизнь) <!-- _fragment_ -->
...
- Сколько ассертов для одного метода

Плюс
- Проверка логгирования и сайд эффектов (ILogger mock)?
...
Да, но есть 2 проблемы:
- Поймаем только первое расхождение
- Должны заранее вручную создать ожидаемый результат
...
Да, но. Нужно переводить в формат csv, парсить, или искать сторонние библиотеки.
...
Отдельный очень мощный и удобный механизм для доступа к структурированным данным, которые обеспечивают типобезопасный доступ без необходимости ручного парсинга данных.

Являются плагинами для компилятора и генерируют типы на основе внешних данных во время компиляции.

---

Провайдер типа в F# позволяет производить парсинг excel в несколько строчек

- Встроенный провайдер типа CSV в FSharp.Data
